...
def predict_image_object_detection_sample(
    ...
    encoded_content = base64.b64encode(content).decode('latin1')

    # Save base64 encoded content to a file for debugging
...
def draw_bounding_boxes(image_path, predictions):
    ...
    for prediction in predictions:
        ...
        for i, bbox in enumerate(bboxes):
            # 바운딩 박스 좌표 변환 (정규화된 값을 이미지 픽셀 좌표로 변환)
            y_min, x_min, y_max, x_max = bbox
            x_min, x_max = x_min * width, x_max * width
            y_min, y_max = y_min * height, y_max * height

            print(f"Original bbox: {bbox}")
            print(f"Transformed bbox: (({x_min}, {y_min}), ({x_max}, {y_max}))")
...
image_path = "YOUR_FILE_PATH"   # 로컬 이미지 경로
